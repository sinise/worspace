//qpackage ie.moguntia.webcrawler;
...
public class SaveURL
{
	 ...
	 */
	public static void saveURL(URL url, Writer writer, String userAgent)
		throws IOException {
			...
			uc.setRequestProperty("Cookie", "AUTH=EMAIL=tony.sadownichik@greenpeace.org&CHALLENGE=US1KIfRUfmcsKeERcCip; mt_user[User][ID]=Q2FrZQ%3D%3D.f0rvCaXH");
		      	System.out.println("trying useragent  "   + userAgent);
		      	uc.connect();
	 ...
	 */
	public static void saveURL(URL url, OutputStream os, String userAgent)
		throws IOException {
	 ...
	 */
	public static String getURL(URL url, String userAgent)
		throws IOException {
		StringWriter sw = new StringWriter();
		saveURL(url, sw, userAgent);
		return sw.toString();
	}
	 ...
	 */
	public static void writeURLtoFile(URL url, String filename, String userAgent)
		throws IOException {
		// FileWriter writer = new FileWriter(filename);
		// saveURL(url, writer);
		// writer.close();
		FileOutputStream os = new FileOutputStream(filename);
		saveURL(url, os, userAgent);
		os.close();
	}
	 ...
	 */
/*	public static Vector extractLinks(URL url, String userAgent)
		throws IOException {
		return extractLinks(getURL(url, userAgent));
	}

	public static Map extractLinksWithText(URL url, String userAgent)
		throws IOException {
		return extractLinksWithText(getURL(url, userAgent));
	}
*/
	/**
...
// 				}
				Set links = extractLinksWithText(url, "Mozilla/5.0 (Windows NT 6.0; rv:33.0) Gecko/20100101 Firefox/33.0").entrySet();
				...
				writeURLtoFile(new URL(args[0]), args[1], userAgent);
				return;
			}
		} catch (Exception e) {
